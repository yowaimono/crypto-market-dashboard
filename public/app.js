...
const STRAT_PRESETS = {
  ...
  // ---------- 融合 / 全天候 ----------
  // ⚠ 注意：在 4h 长周期上这个策略并不可靠。用修正后的引擎回测 2017-2026：
  //   全样本 9 年排名第 1（+9835%），但几乎全靠 2020 年 +280% 撑起来；
  //   最近 4.7 年（2022 起）BTC 只有 -30%，同类里倒数。
  //   即"适应了旧行情"。详见仓库 SWING.md。
  // 保留它是因为在 1m/5m 这类短周期上它可以正常使用（见上方的实盘模拟）。
  fusion: [
  ...
  volfilter: [
    '// ★ 实证最强（4h 长周期）：ATR(7) > ATR(28)（波动放大）且 MA10 > MA30 才做多',
    '// 用波动放大过滤掉无聊的假突破',
    '// 2017-2026 回测（4h，1x，含手续费+滑点）：',
    '//   BTC +804%（年化27.4%）vs 买入持有 +1764%；ETH +2273%（年化41.7%）vs 持有 +754%',
    '//   最大回撤 48%(BTC)/53%(ETH)；6/10 年为正；资产留出 7/8 盈利',
    '//   持仓中位 3.5 天、P75 5.8 天、最长 21 天，68% 落在 2~15 天 —— 适合滚仓波段',
    '//   2022-2026：BTC 年化 +11.3%（持有 +12.4%）/ ETH +14.2%（持有 -7.1%）',
    '// 提醒：ETH 上跑赢持有，BTC 上跑输但回撤减半；1x 回撤已有 48%，别加杠杆',
